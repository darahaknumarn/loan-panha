    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        Try
            ...
            Else
                ...
                addIn("delete from BK_Customer where CM_BrId='" & BrID & "'")
                addIn("delete from BK_Customer1 where CM_BrId='" & BrID & "'")
                addIn("delete from BK_Loan where LD_BrId='" & BrID & "'")
                addIn("delete from BK_Loan1 where LD_BrId='" & BrID & "'")
                addIn("delete from BK_Location where LO_BrId='" & BrID & "'")
                addIn("delete from BK_Location1 where LO_BrId='" & BrID & "'")
                addIn("delete from tblAsset	where BrID='" & BrID & "'")
                addIn("delete from tblAssetAdd where BrID='" & BrID & "'")
                addIn("delete from tblAssetDetail where BrID='" & BrID & "'")
                addIn("delete from tblCollateral where BrID='" & BrID & "'")
                addIn("delete from tblcompany1 where ID='" & BrID & "'")
                addIn("delete from tblCU_Pro where BrID='" & BrID & "'")
                addIn("delete from tblCustom where BrID='" & BrID & "'")
                addIn("delete from tblOum where BrID='" & BrID & "'")
                addIn("delete from tblPosition where BrID='" & BrID & "'")
                addIn("delete from tblResource where BrID='" & BrID & "'")
                addIn("delete from tblSang where BrID='" & BrID & "'")
                addIn("delete from tblStaff where BrID='" & BrID & "'")
                addIn("delete from tblTraceAssetDetail where BrID='" & BrID & "'")
                addIn("delete from tblTraceResource where BrID='" & BrID & "'")
                addIn("delete from tblTracesang where BrID='" & BrID & "'")
                '-------------------------------------------------------------------------------------------Insert into database
                addIn("INSERT INTO BarCode.dbo.BK_Customer SELECT    * FROM BARCODE1.dbo.BK_Customer;")
                addIn("INSERT INTO BarCode.dbo.BK_Customer1 SELECT   * FROM BARCODE1.dbo.BK_Customer1;")
                addIn("INSERT INTO BarCode.dbo.BK_Loan SELECT        * FROM BARCODE1.dbo.BK_Loan;")
                addIn("INSERT INTO BarCode.dbo.BK_Loan1 SELECT       * FROM BARCODE1.dbo.BK_Loan1;")
                addIn("INSERT INTO BarCode.dbo.BK_Location SELECT    * FROM BARCODE1.dbo.BK_Location;")
                addIn("INSERT INTO BarCode.dbo.BK_Location1 SELECT   * FROM BARCODE1.dbo.BK_Location1;")
